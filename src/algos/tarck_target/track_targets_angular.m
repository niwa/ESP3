...
addParameter(p,'Min_max_TS_track',-55,@(ts) (ts>-120&&ts<=-10));
...
id_rem = unique(trans_obj.ST.Track_ID(idx_rem));
idt0 = 0;

if ~isempty(trans_obj.Tracks) && ~isempty(trans_obj.Tracks.target_id)
    ii = find(ismember(trans_obj.Tracks.id,id_rem));
    trans_obj.Tracks.id(ii) = [];
    trans_obj.Tracks.target_id(ii) = [];
    trans_obj.Tracks.target_ping_number(ii) = [];
    trans_obj.Tracks.uid(ii) = [];
    idt0 = min(trans_obj.Tracks.id);
end
...
trans_obj.ST.Track_ID(idx_rem) = nan;
...
data_sz = size(ST.Target_range);

if p.Results.IgnoreAttitude==0
        
    [X_st,Y_st,Z_st] = angles_to_pos_vec(ST.Target_range(:),...
        ST.Angle_major_axis(:),...
        ST.Angle_minor_axis(:),...
        ST.Heave(:),ST.Roll(:),ST.Pitch(:),ST.Yaw(:),...
    ...
    X_st = reshape(X_st,data_sz);
    Y_st = reshape(Y_st,data_sz);
    Z_st = reshape(Z_st,data_sz);
    ...
    [X_st,Y_st,Z_st] = angles_to_pos_vec(ST.Target_range(:),...
        ST.Angle_major_axis(:),...
        ST.Angle_minor_axis(:),...
        zeros(size(ST.Target_range(:))),zeros(size(ST.Target_range(:))),zeros(size(ST.Target_range(:))),zeros(size(ST.Target_range(:))),...
        trans_obj.Config.TransducerAlphaX,trans_obj.Config.TransducerAlphaY,trans_obj.Config.TransducerAlphaZ,...
        trans_obj.Config.TransducerOffsetX,trans_obj.Config.TransducerOffsetY,trans_obj.Config.TransducerOffsetZ,...
        true,true,true,true...
        );
...
pings=min(ST.Ping_number):max(ST.Ping_number);
    ...
    if nb_targets_pings(iip)>0&&id_prev>0
        ...
        while u<=p.Results.Max_Gap_Track&&(iip-1-u)>0
            ...
            if (nb_targets_pings(iip-u)>0&&~isempty(X_s{iip-1-u}))
                target_gate=...
                    (repmat(X_o{iip},nb_targets_pings(iip-1-u),1)-repmat(X_p{iip-u}',1,nb_targets_pings(iip))).^2./((p.Results.ExcluDistMajAxis+repmat(abs(R_o{iip}),nb_targets_pings(iip-1-u),1)*tand(p.Results.MaxStdMajAxisAngle))*(1+u*p.Results.MissedPingExpMajAxis/100)).^2+...
                    (repmat(Y_o{iip},nb_targets_pings(iip-1-u),1)-repmat(Y_p{iip-u}',1,nb_targets_pings(iip))).^2./((p.Results.ExcluDistMinAxis+repmat(abs(R_o{iip}),nb_targets_pings(iip-1-u),1)*tand(p.Results.MaxStdMinAxisAngle))*(1+u*p.Results.MissedPingExpMinAxis/100)).^2+...
                    (repmat(R_o{iip},nb_targets_pings(iip-1-u),1)-repmat(R_p{iip-u}',1,nb_targets_pings(iip))).^2/(p.Results.ExcluDistRange*(1+u*p.Results.MissedPingExpRange/100)).^2;
                ...
                while jjp<length(idx_new_target)
                    jjp=jjp+1;
                    if sum(idx_new_target(jjp)==idx_new_target_tot)==0
                        idx_old_track_temp=[];
                        ...
                        for t=1:length(idx_old_track_temp)
                            if ~any(tracks{idx_old_track_temp(t)}==idx_target{iip}(idx_new_target(jjp)))
                                ...
                                curr_weight=p.Results.WeightMajAxis*(X_o{iip}(idx_new_target(jjp))-Y_p{iip-u}(idx_old_target(jjp))).^2./((p.Results.ExcluDistMajAxis+R_o{iip}(idx_new_target(jjp))*tand(p.Results.MaxStdMajAxisAngle))*(1+u*p.Results.MissedPingExpMajAxis/100)).^2+...
                                    p.Results.WeightMinAxis*(Y_o{iip}(idx_new_target(jjp))-X_p{iip-u}(idx_old_target(jjp))).^2./((p.Results.ExcluDistMinAxis+R_o{iip}(idx_new_target(jjp))*tand(p.Results.MaxStdMinAxisAngle))*(1+u*p.Results.MissedPingExpMinAxis/100)).^2+...
                                    p.Results.WeightRange*(R_o{iip}(idx_new_target(jjp))-R_p{iip-u}(idx_old_target(jjp))).^2/(p.Results.ExcluDistRange*(1+u*p.Results.MissedPingExpRange/100))^2+...
                                    p.Results.WeightTS*diff_TS^2/(delta_TS_max)^2+...
                                    p.Results.WeightPingGap*diff_pings^2/max(p.Results.Max_Gap_Track,1)^2;
                ...
                if ~isempty(idx_new_target)
                    ...
                    VR_o{iip}(idx_new_target)=(R_o{iip}(idx_new_target)-R_p{iip-u}(idx_old_target))/(pings(iip)-pings(id_prev));
...
tracks_out = init_track_struct();

for iip=1:length(tracks)

    idx_targets=tracks{iip};

    if max(ST.TS_comp(idx_targets),[],'all','omitnan')<=p.Results.Min_max_TS_track
        tracks{iip}=[];
        tracks_pings{iip}=[];
        weight{iip}=[];
        continue;
    ...
    if length(idx_targets)>=p.Results.Min_ST_Track&&(max(ST.Ping_number(idx_targets)-min(ST.Ping_number(idx_targets))+1))>=p.Results.Min_Pings_Track
        unique_pings=unique(tracks_pings{iip});

        for t=1:length(unique_pings)
            idx_target_same_ping=find(tracks_pings{iip}==unique_pings(t));
            min_weight=min(weight{iip}(idx_target_same_ping),[],'all','omitnan');
...
tracks_out.id=(1:numel(tracks_out.target_ping_number))+idt0;
tracks_out.uid=generate_Unique_ID(numel(tracks_out.target_ping_number));
tracks_out.Tag = cell(1,numel(tracks_out.uid));
output_struct.tracks=tracks_out;
output_struct.done = true;
...
if isempty(trans_obj.Tracks)
    ...
    ff_tracks = fieldnames(output_struct.tracks);
    for uif = 1:numel(ff_tracks)
        trans_obj.Tracks.(ff_tracks{uif}) = [trans_obj.Tracks.(ff_tracks{uif}) output_struct.tracks.(ff_tracks{uif})];
    end
end
