...
if any(strcmp(layer.Filetype,{'ASL' 'OCULUS' 'DIDSON'}))
...
xl=[min(f_vec_tot,[],'omitnan')/1e3 max(f_vec_tot,[],'omitnan')/1e3];
...
[~,~,f_nom,f_start,f_end]=layer.get_freq_min_max_nom_start_end();
    ...
    calibration_tab_comp.ax_G0.XTick=unique([cell2mat(f_nom) cell2mat(f_start)  cell2mat(f_end)])/1e3;

    for ifif=1:numel(field)
        yd=[calibration_tab_comp.(['ax_' field{ifif}]).Children(:).YData];

        yl=[min(yd,[],'omitnan') max(yd,[],'omitnan')];
        yl(isnan(yl)) = 0;
