...
addParameter(p,'valid_filename',true,@islogical);
...
for ilay=1:nb_layers
    ...
    u=1;
    new_name = sprintf('%s (%s): %s',layers(ilay).Filetype,datestr(t1),new_name);
    new_name_ori=new_name;
    while sum(strcmp(new_name,layers_Str))>=1
    ...
    if p.Results.valid_filename
        layers_Str{ilay} = clean_str(layers_Str{ilay});
    end
