...
switch src.SelectionType
    case {'normal','alt','extend','open'}
        hp=plot(ah,xinit,yinit,'color',cmap_struct.col_lab,'linewidth',1,'Tag','line_temp');
    ...
    otherwise
        [~, idx_line]=min(abs(xinit(1)-xdata));
        [~,idx_r]=min(abs(yinit(1)-ydata));
    ...
    function update_line(x_f,y_f)
        ...
        if length(x_f)>1
            for i=1:length(x_f)-1
                [~, idx_line]=min(abs(x_f(i)-xdata));
                [~, idx_line_1]=min(abs(x_f(i+1)-xdata));
                
                [~,idx_r]=min(abs(y_f(i)-ydata));
                [~,idx_r1]=min(abs(y_f(i+1)-ydata));
        ...
        elseif isscalar(x_f)
            [~, idx_line]=min(abs(x_f-xdata));
            [~,idx_r]=min(abs(y_f-ydata));
            line_obj.Range(idx_line)=Range(idx_r);
        end
    end
