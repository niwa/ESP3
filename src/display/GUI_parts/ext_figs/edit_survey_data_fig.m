...
fields = properties(survey_data_obj);
...
uicontrol(edit_sd_fig,StInfo,'string','Survey name: ','pos',[10 320 90 20],'HorizontalAlignment','right');
survey_handle=uicontrol(edit_sd_fig,EdInfo,'pos',[100 320 100 20],'string',survey_data_obj.SurveyName,'enable',editable{1},'callback',{@up_modified,[],[],[],''},'Tag','SurveyName');

uicontrol(edit_sd_fig,StInfo,'string','Voyage: ','pos',[20 280 80 20],'HorizontalAlignment','right');
voy_handle=uicontrol(edit_sd_fig,EdInfo,'pos',[100 280 80 20],'string',survey_data_obj.Voyage,'enable',editable{2},'callback',{@up_modified,[],[],[],''},'Tag','Voyage');

uicontrol(edit_sd_fig,StInfo,'string','Snapshot: ','pos',[20 240 80 20],'HorizontalAlignment','right');
snap_handle=uicontrol(edit_sd_fig,EdInfo,'pos',[100 240 40 20],'string',num2str(survey_data_obj.Snapshot),'callback',{@up_modified,0,inf,0,'%.0f'},'enable',editable{3},'Tag','Snapshot');

uicontrol(edit_sd_fig,StInfo,'string','Type: ','pos',[20 200 80 20],'HorizontalAlignment','right');
type_handle=uicontrol(edit_sd_fig,LstboxInfo,'pos',[100 200 100 20],'string',type_cell,'value',idx_t,'enable',editable{4},'callback',{@up_modified,[],[],[],''},'Tag','Type');

uicontrol(edit_sd_fig,StInfo,'string','Stratum: ','pos',[20 160 80 20],'HorizontalAlignment','right');
strat_handle=uicontrol(edit_sd_fig,EdInfo,'pos',[100 160 60 20],'string',survey_data_obj.Stratum,'enable',editable{5},'callback',{@up_modified,[],[],[],''},'Tag','Stratum');

uicontrol(edit_sd_fig,StInfo,'string','Transect: ','pos',[20 120 80 20],'HorizontalAlignment','right');
trans_handle=uicontrol(edit_sd_fig,EdInfo,'pos',[100 120 40 20],'string',survey_data_obj.Transect,'callback',{@up_modified,0,inf,0,'%.0f'},'enable',editable{6},'Tag','Transect');

uicontrol(edit_sd_fig,StInfo,'string','Comment: ','pos',[20 80 80 20],'HorizontalAlignment','right');
comment_handle=uicontrol(edit_sd_fig,EdInfo,'pos',[20 40 200 40],'string',survey_data_obj.Comment,'enable',editable{7},'Max',10,'Min',1,'callback',{@up_modified,[],[],[],''},'Tag','Comment');
...
drawnow; % Update the view to remove the closed figure (g1031998)
    function up_modified(src,~,min_val,max_val,deflt_val,precision)
        if ~isempty(min_val)
            check_fmt_box(src,[],min_val,max_val,deflt_val,precision)
        end
        modified = modified | strcmpi(fields,src.Tag);
    end
end
