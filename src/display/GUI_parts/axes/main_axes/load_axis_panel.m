...
axes_panel_comp.axes_panel=uipanel(axes_panel,'Position',[0 0.1 1 0.9]);

load_info_panel(main_figure,uipanel(axes_panel,'Position',[0 0 1 0.1],'BackgroundColor',[1 1 1],'tag','info_panel','visible','on','BorderType','line'));
...
axes_panel_comp.echo_obj.echo_usrdata.ax_tag = 'main';

axes_panel_comp.v_axes_plot=plot(axes_panel_comp.echo_obj.vert_ax,nan,nan,'Color',[0.8 0 0]);
axes_panel_comp.v_bot_val=yline(axes_panel_comp.echo_obj.vert_ax,0,'Color',cmap_struct.col_bot,'Tag','bot_val','Interpreter','none');
axes_panel_comp.v_curr_val=yline(axes_panel_comp.echo_obj.vert_ax,0,'LineStyle','--','Color',circshift(cmap_struct.col_bot,1),'Tag','curr_val','Interpreter','none');       
...
axes_panel_comp.h_curr_val=xline(axes_panel_comp.echo_obj.hori_ax,0,'LineStyle','--','Color',circshift(cmap_struct.col_bot,1),'Tag','curr_val','Interpreter','none');       
...
function enter_panel(figHandle, ~)
    ...
    replace_interaction(figHandle,'interaction','WindowScrollWheelFcn','id',1,'interaction_fcn',@scroll_fcn_callback);
end

function exit_bottom_plot_fcn(~,~,hplot)
set(hplot,'linewidth',0.5);
end

function enter_bottom_plot_fcn(src,~,hplot)
