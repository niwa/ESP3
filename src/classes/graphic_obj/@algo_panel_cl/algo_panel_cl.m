    ...
    methods
        function obj=algo_panel_cl(varargin)
            ...
            obj.algo=results.algo;
             ...
             nb_c = max(ceil((nb_params-nb_bool_params+1)/nb_r_max),nb_c_min);
            ...
            if isempty(results.title)
                obj.title = obj.algo.Display_name;
            else
                ...
                if isempty(results.input_struct_h) || ~isfield(results.input_struct_h,param_names{ui})
                    ...
                    switch params_class{ui}
                        ...
                        case 'logical'
                            ...
                            if numel(param_names)>ui&&(~strcmpi(params_class{ui+1},'logical')&&isempty(obj.algo.Input_params(ui+1).Disp_name))
                                ip = ip-1;
                            end
            ...
            x_m = max(arrayfun(@(x) max(x.Position(1)+x.Position(3)),obj.container.Children))+2*gui_fmt.x_sep;
            if obj.container.Position(3)<x_m
                obj.container.Position(3)  = x_m;
            end
            
            function load_params_fcn(src,~)
            ...
            function update_algo_input_param_fcn(src,~)
        ...
        function reset_range_algo_panels(obj,range)
            for ui = 1:numel(obj)
                obj(ui).algo.reset_range(range);
                obj(ui).update_algo_panel([]);
            end
        end
