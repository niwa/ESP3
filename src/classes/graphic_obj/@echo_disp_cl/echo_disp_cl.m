classdef echo_disp_cl < handle
    ...
    properties
        ...
        v_plot matlab.graphics.chart.primitive.Line
        v_curr_val matlab.graphics.chart.decoration.ConstantLine
        v_bot_val matlab.graphics.chart.decoration.ConstantLine
        h_plot_high  matlab.graphics.chart.primitive.Line
        h_plot_low  matlab.graphics.chart.primitive.Line
        h_curr_val matlab.graphics.chart.decoration.ConstantLine
        h_name matlab.graphics.primitive.Text
    ...
    methods
        function obj = echo_disp_cl(parent_h,varargin)
            ...
            addRequired(p,'parent_h',@(x) isa(x,'matlab.ui.container.Tab')||isa(x,'matlab.ui.container.Panel')...
                ||isa(x,'matlab.ui.Figure')||isa(x,'matlab.ui.container.GridLayout')||isa(x,'matlab.graphics.layout.TiledChartLayout')||isempty(x));
            addParameter(p,'geometry_x','pings',@(x) ismember(x,{'pings','meters'}));
            addParameter(p,'geometry_y','samples',@(x) ismember(x,{'samples','depth','range'}));
            ...
            addParameter(p,'disp_minor_grid','off',@(x) ismember(x,{'off','on'}));
            ...
            cmap_struct = init_cmap(p.Results.cmap);
            ...
            if ~isempty(p.Results.CID)
                obj.echo_usrdata.CID = p.Results.CID;
            end
            ...
            obj.main_ax=feval(ax_fcn,...
                ...
                'XMinorGrid',p.Results.disp_minor_grid,...
                'YMinorGrid',p.Results.disp_minor_grid,...
                'GridLineStyle','--',...
                'MinorGridLineStyle',':',...
                'NextPlot','add',...
                'YDir',p.Results.YDir,...
            ...
            obj.light_h = light(obj.main_ax,'Visible','off');
            ...
            if p.Results.disp_vert_ax
                ...
                if  ismember(obj.echo_usrdata.geometry_y,{'depth' 'range'})
            ...
            obj.h_name = text(obj.main_ax,[],[],'',...
            'Units','Pixel','Fontweight','normal','Fontsize',p.Results.FontSize+2,...
            'Tag',p.Results.ax_tag,'UserData',obj.echo_usrdata);
            ...
            set(obj.echo_surf,...
                ...
                'AlphaData',zeros(size(echo_init),'uint8'),...
                'LineStyle','none',...
            ...
            obj.bottom_line_plot=plot(obj.main_ax,nan,nan,'Tag','bottom','Color',cmap_struct.col_bot);

            if p.Results.disp_vert_ax
                obj.v_plot=plot(obj.vert_ax,nan,nan,'Color',[0.8 0 0]);
                obj.v_bot_val=yline(obj.vert_ax,0,'Color',cmap_struct.col_bot,'Tag','bot_val','Interpreter','none');
                obj.v_curr_val=yline(obj.vert_ax,0,'LineStyle','--','Color',circshift(cmap_struct.col_bot,1),'Tag','curr_val','Interpreter','none');
            end
            if p.Results.disp_hori_ax
                obj.h_plot_low=plot(obj.hori_ax,nan,nan,'color',[0 0.5 0]);
                obj.h_plot_high=plot(obj.hori_ax,nan,nan,'color',[0.5 0 0],'linestyle','-','marker','o','MarkerFaceColor',[1 1 1]-cmap_struct.col_ax);
                obj.h_curr_val=xline(obj.hori_ax,0,'LineStyle','--','Color',circshift(cmap_struct.col_bot,1),'Tag','curr_val','Interpreter','none');
            end
        ...
        function update_echo_usrdata(obj,userdata)      
            f_to_update = {'h_name' 'echo_surf' 'echo_bt_surf'};
            for uif = 1:numel(f_to_update)
                if ~isempty(obj.(f_to_update{uif}))
                    obj.(f_to_update{uif}).UserData = userdata;
                end
            end
           obj.echo_usrdata = userdata;
        end
        ...
        function order_echo_stack(obj,varargin)
            ...
            select_area_t=findobj(obj.main_ax,'tag','SelectArea');
            select_area=getappdata(ancestor(obj.main_ax,'Figure'),'SelectArea');
            if ~isempty(select_area_t)&&~isempty(select_area)
        ...
        function update_echo_grid(obj,trans_obj,varargin)
            ...
            idx_ping=ceil(obj.echo_usrdata.Idx_ping);

            xdata = obj.echo_surf.XData(1,:);
            ...
            switch curr_disp.Xaxes_current
                ...
                case 'pings'
                    xdata_grid=trans_obj.get_transceiver_pings(idx_ping,1);
                case 'meters'
            ...
            obj.main_ax.XTick=xdata(idx_xticks);
            obj.main_ax.XAxis.MinorTickValues=xdata(idx_minor_xticks);
            ...
            switch obj.echo_usrdata.geometry_y
                ...
                otherwise
                    ...
                    ydata_grid=trans_obj.get_samples_range(idx_r);
            end

            if dy == 0
                dy = mean(diff(ydata_grid)/10);
            end
            ...
            switch lower(curr_disp.Xaxes_current)
                case 'seconds'
                    if range(xdata_grid(idx_xticks))/numel(idx_xticks) >= (24*60*60)/2
                        h_fmt = 'dd mmm';
                    elseif range(xdata_grid(idx_xticks))/numel(idx_xticks) >= (60*60)/2
                        h_fmt='HH:MM';
                    else
                         h_fmt='HH:MM:SS';
                    end
                ...
                case 'meters'
                     if range(xdata_grid(idx_xticks))/numel(idx_xticks) >= 1e3
                            xl = cellfun(@(x) x/1852,xl,'un',false);
                            fmt=[str_start '%.1fnm'];
                     else
                         fmt=[str_start '%.0fm'];
                     end
        ...
        function  set_echo_alphamap(obj,trans_obj,varargin)
            ...
            addParameter(p,'curr_disp',[],@(x) isa(x,'curr_state_disp_cl')||isempty(x));
            parse(p,obj,trans_obj,varargin{:});

            curr_disp = p.Results.curr_disp;
            cax = curr_disp.getCaxField(obj.echo_usrdata.Fieldname);
            [~,~,~,AlphaDisp]=init_cax(obj.echo_usrdata.Fieldname);
            min_axis=cax(1);
            ...
            if p.Results.update_under_bot>0
                alpha_map=ones(size(data),prec)*numel(curr_disp.get_alphamap());
                func_bot = @le;
                switch obj.echo_usrdata.geometry_y
                    case'samples'
                        bot_vec_red = trans_obj.get_bottom_idx(idx_ping,idx_beam);
                        bot_vec_red(bot_vec_red == 1) = nan;
                        bot_vec_red = bot_vec_red + 1/2;
                    case 'depth'
                        [roll_comp_bool,pitch_comp_bool,yaw_comp_bool,heave_comp_bool] = trans_obj.get_att_comp_bool();
                        [data_struct,~]=trans_obj.get_xxx_ENH('data_to_pos',{'bottom'},'idx_ping',idx_ping,'idx_r',[],'idx_beam',idx_beam,...
                            ...
                            'no_nav',true);
                        bot_vec_red = data_struct.bottom.H;
                        if trans_obj.get_transducer_pointing_angle() < 0
                               func_bot = @ge;
                        end
                    case 'range'
                        bot_vec_red = trans_obj.get_bottom_range(idx_ping,idx_beam);
                end

                if size(bot_vec_red,3) >  1
                    bot_vec_red = squeeze(bot_vec_red(:,:,round(size(bot_vec_red,3)/2)));
                end
                
                idx_bot_red=bsxfun(func_bot,mean(bot_vec_red,3,'omitnan'),ydata);
            ...
            if ~isempty(trans_obj.Features)
                mask_ft = trans_obj.get_feature_mask('idx_ping',idx_ping,'idx_r',idx_r,'idx_beam',idx_beam);
                if size(mask_ft,3)>1
                    mask_ft = any(mask_ft,3);
                end
                alpha_map(~mask_ft & alpha_map == numel(curr_disp.get_alphamap())) = 6;
            end
            ...
            if p.Results.update_bt>0
                ...
                alpha_map_bt = ones(size(data),prec);
            ...
            switch AlphaDisp
                case {'SpCaxBounds' 'CorrCaxBounds'}
                    ...
                    for uif = 1:numel(fields)
                        field = fields{uif};
                        data_sec = trans_obj.Data.get_subdatamat('idx_r',idx_r,'idx_ping',idx_ping,'field',field,'idx_beam',idx_beam);
                        if size(data_sec,3)>1
                            data_sec = max(data_sec,[],3);
                        end
                        cax_sec = curr_disp.getCaxField(field);
                        if ~isempty(data_sec)
                            alpha_map(data_sec<cax_sec(1)) = 2;
                        end
                    end
                case 'InfBounds'
        ...
        function [ping_range,sample_range] = get_ping_sample_range(echo_obj,trans_obj)
            ...
            [~,p_min] = min(abs(xdata-min(echo_obj.echo_surf.XData(:))));
            [~,p_max] = min(abs(xdata-max(echo_obj.echo_surf.XData(:))));
            ...
            switch echo_obj.echo_usrdata.geometry_y
                ...
                case 'depth'
                    ydata=trans_obj.get_samples_depth([],[]);
                case 'range'
                    ydata=trans_obj.get_samples_range();
            end

            [~,s_min] = min(abs(ydata-min(echo_obj.echo_surf.YData(:))));
            [~,s_max] = min(abs(ydata-max(echo_obj.echo_surf.YData(:))));
        ...
        function ping_range = get_ping_range(echo_obj,trans_obj)
            ...
            switch echo_obj.echo_usrdata.geometry_x
                case 'pings'
                    xdata=trans_obj.get_transceiver_pings()-1/2;
                case 'seconds'
                    xdata=trans_obj.get_transceiver_time();
                case 'meters'
                    xdata=trans_obj.GPSDataPing.Dist;
        ...
        function display_echo_bottom(obj,trans_obj,varargin)
            ...
            if ~isempty(trans_obj.Bottom.Sample_idx)
                idx_bottom = trans_obj.get_bottom_idx(idx_ping,idx_beam);
                
                switch obj.echo_usrdata.geometry_x
                    case 'pings'
                        xdata = trans_obj.get_transceiver_pings(idx_ping);
                    case 'seconds'
                        xdata = trans_obj.get_transceiver_time(idx_ping);
                    case 'meters'
                        xdata = trans_obj.GPSDataPing.Dist(idx_ping);
                end
                ...
                switch obj.echo_usrdata.geometry_y
                    ...
                    case 'depth'
                        ...
                        [data_struct,~]=trans_obj.get_xxx_ENH('data_to_pos',{'bottom'},'idx_ping',idx_ping,'idx_r',[],'idx_beam',idx_beam,...
                            ...
                            'heave_comp',heave_comp_bool,...
                            'no_nav',true);
                        y = data_struct.bottom.H;
                    case 'range'
                        y=trans_obj.get_bottom_range(idx_ping,idx_beam);
                end

                if size(y,3) > 1 
                    y = squeeze(y(:,:,round(size(y,3)/2)));
                end
        ...
        function [pos_m,pos_v,pos_h,pos_cb] = get_axes_position(obj,vpos_r,hpos_r)
            ...
            pos_cb(3)=width_colorbar*1/4;
            pos_cb(1)=pos_cb(1)+width_colorbar*1/3;

            height_col=pos_cb(4);

            pos_cb(2)=pos_m(2)+height_col*0.05;
            pos_cb(4)=height_col*0.9;

        end

        function disp_basic_echo(echo_obj,trans_obj,curr_disp,field,x,y,x_lim,y_lim)

            [~,~,~] =echo_obj.display_echogram(trans_obj,...
                'curr_disp',curr_disp,...
                'Fieldname',field,...
                'x',x,...
                'y',y,...
            ...
            new_fig.Alphamap = curr_disp.get_alphamap();

            if isempty(x_lim)
                x_lim=[min(echo_obj.echo_surf.XData(:)) max(echo_obj.echo_surf.XData(:))];
            end

            if isempty(y_lim)
                y_lim =[min(echo_obj.echo_surf.YData(:)) max(echo_obj.echo_surf.YData(:))];
            end
            ...
            echo_obj.order_echo_stack('bt_on_top',1);

            [~,~,Units]=init_cax(curr_disp.Fieldname);
            echo_obj.colorbar_h.Label.String = Units;
        end
