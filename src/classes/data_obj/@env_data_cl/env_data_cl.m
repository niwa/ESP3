...
classdef env_data_cl < matlab.mixin.Copyable
    ...
    methods
        function obj = env_data_cl(varargin)
            ...
            addParameter(p,'AttModel','fandg',@ischar);
            ...
            addParameter(p,'CTD_fname',[],@ischar);
            parse(p,varargin{:});
            ...
            for i=1:length(props)
                switch props{i}
                    ...
                    case 'CTD_fname'
                        if ~strcmp(results.CTD_fname,'CTD_fname')&&ischar(results.CTD_fname)
                            obj.CTD_fname=results.CTD_fname;
                            obj.load_ctd(results.CTD);
                        end
                    otherwise
        ...
        function load_ctd(env_obj,fname)
            if ~isempty(env_obj.CTD_fname)
                fname = env_obj.CTD_fname;
            end
            ...
            if isfile(fname)
                try
                    ...
                    env_obj.CTD = struct('depth',[],'temperature',[],'salinity',[],'ori','profile');
                    env_obj.CTD.depth = tabs.depth;
                    env_obj.CTD.temperature = tabs.temperature;
                    env_obj.CTD.salinity = tabs.salinity;
        ...
        function env_data_struct = env_data_to_struct(obj)
            props = properties(obj);
            env_data_struct = struct();
            for ip = 1:numel(props)
                env_data_struct.(props{ip}) = obj.(props{ip});
            end
        end
    end
end