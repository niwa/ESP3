...
switch class(Ext_obj)
    case 'mbs_cl'
        for i=1:nb_trans
            ...
            obj.LatLim(1)=min(obj.LatLim(1),min(obj.SliceLat{i}));
            obj.LongLim(1)=min(obj.LongLim(1),min(obj.SliceLong{i}));
            obj.LatLim(2)=max(obj.LatLim(2),max(obj.SliceLat{i}));
            obj.LongLim(2)=max(obj.LongLim(2),max(obj.SliceLong{i}));
    ...
    case 'layer_cl'
            ...
            if p.Results.SliceSize>0
                ...
                for itout=1:numel(output_type)
                    if ~isempty(slice_int)
                        slice_int=slice_int+sum(output_2D{itout}.eint,'omitnan');
                    else
                        slice_int=sum(output_2D{itout}.eint,'omitnan');
                    end
                    good_pings=max(good_pings,max(output_2D{itout}.Nb_good_pings,[],1));
                end
        ...
        for it=1:nb_trans
            if ~isempty(obj.Lat{it})
                obj.LatLim(1)=min(obj.LatLim(1),min(obj.Lat{it}));
                obj.LongLim(1)=min(obj.LongLim(1),min(obj.Long{it}));
                obj.LatLim(2)=max(obj.LatLim(2),max(obj.Lat{it}));
                obj.LongLim(2)=max(obj.LongLim(2),max(obj.Long{it}));
            end
        end
    ...
    case 'survey_cl'
        ...
        for ireg=1:length(survey_obj.SurvOutput.regionSum.tag)
            ...
            obj.Regions.Lat_m(ireg)=mean(survey_obj.SurvOutput.regionSumAbscf.latitude{ireg});
            obj.Regions.Long_m(ireg)=mean(survey_obj.SurvOutput.regionSumAbscf.longitude{ireg});
        end
    case {'struct' 'gps_data_cl'}
        ...
        lat_lim(1)=min(lat_lim(1),min(lat));
        lon_lim(1)=min(lon_lim(1),min(long));
        lat_lim(2)=max(lat_lim(2),max(lat));
        lon_lim(2)=max(lon_lim(2),max(long));
