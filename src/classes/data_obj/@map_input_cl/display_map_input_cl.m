...
for ui=1:numel(obj_tot)
    ...
    if ~strcmp(field,'Tag')
        ...
        for usnap=1:nb_snap
            ...
            val_max=max(cellfun(@max,vals(~cellfun(@isempty,vals))));
            ...
            for uui=1:length(idx_snap)
                ...
                if ~isempty(obj.Long{idx_snap(uui)})
                    ...
                    if ~isempty(obj.StationCode{idx_snap(uui)})
                        text(mean(obj.Lat{idx_snap(uui)}),mean(obj.Long{idx_snap(uui)}),obj.StationCode{idx_snap(uui)},'parent',n_ax(usnap),'color','r');
                    end
                end
        ...
        for utag=1:length(tag)
            ...
            if strcmp(field,'Tag')
                ...
                for itrans=1:length(unique_trans)
                    ...
                    text(mean(obj.Regions.Lat_m(ireg_tag(itrans_curr))),mean(obj.Regions.Long_m(ireg_tag(itrans_curr))),tag{utag},'parent',n_ax(utag),'color','r');
                    text(mean(obj.Regions.Lat_m(ireg_tag(itrans_curr))),mean(obj.Regions.Long_m(ireg_tag(itrans_curr))),sprintf('\n,%.0f',obj.Regions.Transect(ireg_tag(itrans_curr(1)))),'parent',n_ax(utag),'color','b');
                end
            end
