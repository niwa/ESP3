...
addRequired(p,'active_reg',@(x) isempty(x)||isa(x,'region_cl'));
...
for u=1:numel(active_reg)
    ...
    for ir=1:length(idx_freq_sort)
        ...
        data_struct_new = data_struct_new.field;
        if u==1
    ...
    case '.vrml'
        for ifif=1:length(idx_freq_sort)
            ...
            writeVrmlPoint_chs(...
                ...
                'thr_max',max(data_tot.(sprintf('data%d',ifif)).data_disp(idx_keep)),'cmap',p.Results.cmap);
        end
end
disp('Done');
