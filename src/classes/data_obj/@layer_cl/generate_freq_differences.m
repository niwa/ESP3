...
for ireg=1:numel(reg_primary)
    [regs_secondary,idx_freqs_secondary,r_factor,~]=layer_obj.generate_regions_for_other_freqs(idx_freq_primary,reg_primary(ireg),idx_freqs_secondary);
    
    if any(r_factor*reg_primary(ireg).Cell_h<2)
            ss_ori= reg_primary(ireg).Cell_h;
           reg_primary(ireg).Cell_h=min(ceil(1/r_factor))*2;
           warning('Could not subsample at %d, doing it at %d instead',ss_ori,reg_primary(ireg).Cell_h);
           [regs_secondary,idx_freqs_secondary,~,~]=layer_obj.generate_regions_for_other_freqs(idx_freq_primary,reg_primary(ireg),idx_freqs_secondary);
    ...
    output_reg_primary=trans_obj_primary.integrate_region(reg_primary(ireg),'keep_bottom',1,'keep_all',1,'sv_thr',p.Results.sv_thr,'load_bar_comp',load_bar_comp,...
        'cal',cal_fm{idx_freq_primary},'envdata',layer_obj.EnvData);

    output_regs_secondary=cell(1,numel(idx_freqs_secondary));
    cax=nan(numel(idx_freqs_secondary),2);
    
    for ifreq=1:numel(idx_freqs_secondary)
        trans_obj_secondary=layer_obj.Transceivers(idx_freqs_secondary(ifreq));
        output_regs_secondary{ifreq}=trans_obj_secondary.integrate_region(regs_secondary(ifreq),'keep_bottom',1,'keep_all',1,'sv_thr',p.Results.sv_thr,'load_bar_comp',load_bar_comp,...
            'cal',cal_fm{idx_freqs_secondary(ifreq)},'envdata',layer_obj.EnvData);
        output_diff{ireg,ifreq}  = substract_reg_outputs(output_reg_primary,output_regs_secondary{ifreq});
        
        if isempty(p.Results.region)
            trans_obj_primary.set_sv_diff(output_diff{ireg,ifreq},secondary_freqs(ifreq));
        end
        
        sv=pow2db_perso(output_diff{ireg,ifreq}.sv(:));
        cax(ifreq,1)=prctile(sv(sv>-999),10)-5;
        cax(ifreq,2)=prctile(sv(sv>-999),90)+5;
    end
    cax(isnan(cax(:,1)),:)=[];
end
