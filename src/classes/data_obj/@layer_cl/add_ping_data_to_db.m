...
%clear_existing_data = 1;
[path_lays,files_lays]=layers_obj.get_path_files();
[path_lays_unique,~,id_u] = unique(path_lays);

for iup = 1:numel(path_lays_unique)
    idx_lays  = layers_obj.find_layer_idx_files(files_lays(iup == id_u));
    if isempty(path_lays_unique{iup})||~isfolder(path_lays_unique{iup})
        continue;
    end 
    try
        db_file = fullfile(path_lays_unique{iup},'echo_logbook.db');
        if ~isfile(db_file)
            dbconn = initialize_echo_logbook_dbfile(path_lays_unique{iup},0);
        else
        ...
        for ilay=idx_lays
            ...
            for itrans=itrans_tot
                ...
                freq=floor(layers_obj(ilay).Frequencies(itrans));
                ...
                for ip=1:length(layers_obj(ilay).Filename)
                    ...
                    [~,fileOri,extN]=fileparts(layers_obj(ilay).Filename{ip});
                    ...
                    if clear_existing_data>0
                        clearPingTable(dbconn,[fileOri extN],freq,[layers_obj(ilay).Transceivers(itrans).Time(1) layers_obj(ilay).Transceivers(itrans).Time(end)]);
                    end

                    %time_cell=datetime(gps_data_obj.Time(id_keep_f),'ConvertFrom','datenum');
                    %time_cell=datestr(gps_data_obj.Time(id_keep_f),'yyyy-mm-dd HH:MM:SS');
                    ...
                    try
                        ...
                        try
                            datainsert_perso(dbconn,'ping_data',t);
                        catch
                            fprintf('Updating ping data structure to latest version\n');
