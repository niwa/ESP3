...
layers_Str=list_layers(layer_obj,'nb_char',80,'valid_filename',true);
output_f_def=fullfile(path_tmp,layers_Str{1});

p = inputParser;

addRequired(p,'layer_obj',@(layer_obj) isa(layer_obj,'layer_cl'));
addParameter(p,'idx_main_freq',[],@isnumeric);
...
if isempty(p.Results.idx_main_freq)
    idx_main_freq = layer_obj.EchoIntStruct.idx_freq_out;
else
...
idx_main=find(ismember(idx_main_freq,idx_freq_out));
...
if ~isempty(idx_main)
    ...
    for uim = 1:numel(idx_main)
        for ir=1:numel(output_2D_type{idx_main(uim)})
            if ~isempty(output_2D{idx_main(uim)}{ir})
                fname=generate_valid_filename([p.Results.output_f '_' output_2D_type{idx_main(uim)}{ir} '_' num2str(layer_obj.Frequencies(idx_main_freq(idx_main(uim)))) '_sliced_transect.csv']);
                
                reg_output_table=reg_output_to_table(output_2D{idx_main(uim)}{ir});
