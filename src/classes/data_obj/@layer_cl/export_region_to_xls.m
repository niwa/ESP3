...
[p_n,f_n,ext] = fileparts(p.Results.output_f);
...
for u = 1:numel(active_reg)
    ...
    for ir = 1:length(idx_freq_sort)
        ...
        % integrate region
        output_reg = layer_obj.Transceivers(idx_freq_sort(ir)).integrate_region(regs(ir),'envdata',layer_obj.EnvData,'cal',cal_fm{idx_freq_sort(ir)});
        reg_output_table = reg_output_to_table(output_reg);
        
        % write integration results to output excel file
        switch ext
            case '.csv'
                writetable(reg_output_table,fullfile(p_n,sprintf('%s%.0f_kHz.csv',f_n,layer_obj.Frequencies(idx_freq_sort(ir))/1e3)));
            case '.xlsx'
        ...
        output_reg.sv(output_reg.sv==0) = NaN;
        Sv_mean = pow2db_perso(mean(output_reg.sv(:)));
        delta_sv = nanstd(pow2db_perso(output_reg.sv(:)));
...
switch ext
    case '.csv'
        writetable(reg_output_table,fullfile(p_n,sprintf('%s_descriptors.csv',f_n)));
    case '.xlsx'
