...
for ix = 1:length(bot_file_str)
    ...
    for itrans = 1:length(bottom_xml_tot)
        ...
        if ~contains(deblank(trans_obj.Config.ChannelID),bottom_xml.Infos.ChannelID)
        ...
        if init_bot(idx_freq)==1
            ...
            new_bottom{idx_freq} = bottom_cl(...
                'Sample_idx',nan(max(layer_obj.Transceivers(idx_freq).Data.Nb_beams),numel(layer_obj.Transceivers(idx_freq).Data.FileId)),...
        ...
        switch bot_fmt_ver
            case '0.1'
                ...
                if time(1)<=trans_obj.Time(1)
                    [~,idx_ping_start] = min(abs(trans_obj.Time(1)-time(1)));
                    idx_start_file = 1;
                else
                    idx_ping_start = 1;
                    [~,idx_start_file] = min(abs(trans_obj.Time-time(1)));
                end
                
                if time(end)>=trans_obj.Time(end)
                    [~,idx_ping_end] = min(abs(trans_obj.Time(end)-time));
                    idx_end_file = length(trans_obj.Time);
                else
                    idx_ping_end = length(time);
                    [~,idx_end_file] = min(abs(trans_obj.Time-time(end)));
                end
                ...
                sample_idx = resample_data_v2((1:length(trans_obj.get_samples_range())),trans_obj.get_samples_range(),depth_resampled,'Opt','Nearest');
                ...
                nb_val=min(numel(sample_idx),numel(idx_start_file:idx_end_file));
            ...
            case '0.2'
                ...
                nb_val=min(numel(samples),numel(new_bottom{idx_freq}.Sample_idx));
            ...
            case '0.3'
                ...
                if ~isempty(pings)
                    ...
                    new_bottom{idx_freq}.Sample_idx(pings+iping_ori-1) = samples(1:nb_val);
                    new_bottom{idx_freq}.Tag(pings+iping_ori-1)        = tag(1:nb_val);
                    new_bottom{idx_freq}.Bottom_params.E1(pings+iping_ori-1)         = E1(1:nb_val);
                    new_bottom{idx_freq}.Bottom_params.E2(pings+iping_ori-1)         = E2(1:nb_val);
                end

            case '0.4'
                pings = bot_xml.Ping;
                if isempty(pings)||isempty(new_bottom{idx_freq})
                    continue;
                end

                samples = reshape(bot_xml.Sample,numel(pings),numel(bot_xml.Sample)/numel(pings))';
                tag     = bot_xml.Tag;
                E1      = bot_xml.E1;
                E2      = bot_xml.E2;
                iping_ori = find(layer_obj.Transceivers(idx_freq).Data.FileId==ix,1);
                nb_val=min(numel(pings),size(new_bottom{idx_freq}.Sample_idx,2));

                for uib = 1:size(samples,1)
                    samples_tmp = samples(uib,:);
                    samples_tmp(samples_tmp<=1) = nan;
                    new_bottom{idx_freq}.Sample_idx(uib,pings+iping_ori-1) = samples_tmp(1:nb_val);
                end

                new_bottom{idx_freq}.Tag(1,pings+iping_ori-1)        = tag(1:nb_val);
                new_bottom{idx_freq}.Bottom_params.E1(1,pings+iping_ori-1)         = E1(1:nb_val);
                new_bottom{idx_freq}.Bottom_params.E2(1,pings+iping_ori-1)         = E2(1:nb_val);
        end
    end
end
