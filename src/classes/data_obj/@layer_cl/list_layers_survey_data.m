...
for it=1:length(layers)
    ...
    if ~isempty(layers(it).SurveyData)
        ...
        output.Type{nb_survd}=survd.Type;
        output.Transect(nb_survd)=survd.Transect;
        if isempty(layers(it).GPSData)||isempty(layers(it).GPSData.Time)
            output.StartTime(nb_survd)=layers(it).Transceivers(1).Time(1);
            output.EndTime(nb_survd)=layers(it).Transceivers(1).Time(end);
        else
            output.StartTime(nb_survd)=layers(it).GPSData.Time(1);
            output.EndTime(nb_survd)=layers(it).GPSData.Time(end);
        end
        output.Layer_idx(nb_survd)=it;
        output.Folder{nb_survd}=curr_folder;
