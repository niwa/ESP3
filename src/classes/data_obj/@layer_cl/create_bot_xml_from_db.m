...
for ifile = 1:length(path_xml)
    ...
    if exist(dbfile,'file')==0
        ...
        bot_xml_version = dbconn.fetch(sprintf('select Bot_XML,Version from bottom where instr(Filename, ''%s'')>0',bot_file_str{ifile}));
        ...
        if isempty(bot_xml_version)           
            ...
            ver_num = bot_xml_version.Version;
            
            if p.Results.bot_ver==0
                % requesting latest version
                [ver_bot,idx_xml] = max(ver_num);
            else
            ...
            xml_str = bot_xml_version.Bot_XML{idx_xml};
