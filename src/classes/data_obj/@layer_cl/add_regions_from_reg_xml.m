...
for ix=1:length(reg_file_str)
    ...
    for itrans=1:length(region_xml_tot)
        ...
        if isempty(trans_obj) || ~isfield(region_xml,'Regions')
            continue;
        end
        
        if ~contains(deblank(trans_obj.Config.ChannelID),region_xml.Infos.ChannelID)
        ...
        t_max=max(trans_obj.Time);
        t_min=min(trans_obj.Time);
        ...
        for i=1:length(reg_xml)
            ...
            switch ver
                case '0.1'
                    ...
                    samples=resample_data_v2(1:length(trans_obj.get_samples_range()),trans_obj.get_samples_range(),depth_box,'Opt','Nearest');
                    
                    Idx_ping=pings(1):pings(2);
                    Idx_r=samples(1):samples(2);
                    
                    if all(isnan(Idx_ping))
                        continue;
                    end
                    ...
                    switch lower(Shape)
                        ...
                        case 'polygon'
                            ...
                            for ic=1:length(reg_xml{i}.Contours)
                                ...
                                Y_cont{i_cont}=resample_data_v2(1:length(trans_obj.get_samples_range()),trans_obj.get_samples_range(),reg_xml{i}.Contours{ic}.Range,'Opt','Nearest');
