function write_bot_to_bot_xml(layer_obj,varargin)
...
for ifile = 1:length(bot_file_str)
    ...
    if exist(xml_file,'file') == 2
        ...
        for ib = 1:length(bottom_xml_old)
            if ~ismember(bottom_xml_old{ib}.Infos.ChannelID,layer_obj.AvailableChannelIDs)
                continue;
            end
    ...
    for itr = 1:length(layer_obj.Transceivers)
        docNode = layer_obj.Transceivers(itr).create_trans_bot_xml_node(docNode,ifile,bot_fmt_ver);
    end
