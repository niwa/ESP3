...
for ifile=1:length(path_xml)
    ...
    if p.Results.reg_ver>=0
        reg_xml_version = dbconn.fetch(sprintf('select Reg_XML,Version from region where instr(Filename, ''%s'')>0',reg_file_str{ifile}));
        ...
        ver_num=reg_xml_version.Version;
        
        if p.Results.reg_ver==0
            [ver_reg_loaded,idx_xml]=max(ver_num);
        else
            idx_xml=find(ver_num<=p.Results.reg_ver,1,'last');
            ver_reg_loaded=ver_num(idx_xml);
        end

        xml_str=reg_xml_version.Reg_XML{idx_xml};
