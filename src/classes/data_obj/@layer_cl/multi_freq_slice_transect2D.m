...
addParameter(p,'regs',region_cl.empty(),@(x) isempty(x)||isa(x,'region_cl'));
...
for irt=1:numel(regions_init)
    [regs_end,idx_freq_end,r_fac,t_fac] = layer_obj.generate_regions_for_other_freqs(p.Results.idx_main_freq,regions_init(irt),p.Results.idx_sec_freq);
    regs=[regions_init(irt) regs_end];
    r_fac=[1 r_fac];
    t_fac=[1 t_fac];
    [idx_freq_out{irt},is]=sort([p.Results.idx_main_freq,idx_freq_end]);
    regs_out{irt}=regs(is);
    r_factor{irt}=r_fac(is);
    t_factor{irt}=t_fac(is);
end
...
dt_main=mean(diff(t_main));
cal_fm = layer_obj.get_fm_cal(idx_freq_out_tot);

num_f_fm=0;
for i_freq=1:numel(idx_freq_out_tot)
    if strcmpi(layer_obj.Transceivers(idx_freq_out_tot(i_freq)).Mode,'FM')
        num_f_fm = num_f_fm+1;
    end
end
comptb = 1;
...
for i_freq=1:numel(idx_freq_out_tot)
    ...
    dt_new=mean(diff(t_new),'omitnan');
    surv_options_obj_cp=copy_survey_option(surv_options_obj);

    switch surv_options_obj.Vertical_slice_units.Value
        case 'pings'
            surv_options_obj_cp.Vertical_slice_size.set_value(floor(p.Results.survey_options.Vertical_slice_size.Value* dt_main/dt_new));
        case {'meters' 'seconds'}
            surv_options_obj_cp.Vertical_slice_size.set_value(p.Results.survey_options.Vertical_slice_size.Value);
    end
    
        
    trans_obj=layer_obj.Transceivers(idx_freq_out_tot(i_freq));
    old_fs = trans_obj.Params.FrequencyStart;
    old_fe = trans_obj.Params.FrequencyEnd;
    try
        if size(surv_options_obj.FrequenciesMinToEI_FMmode.Value,2) == num_f_fm
            switch trans_obj.Mode
                case 'FM'
                    if (surv_options_obj.FrequenciesMinToEI_FMmode.Value(comptb) >= trans_obj.Params.FrequencyStart)||(surv_options_obj.FrequenciesMinToEI_FMmode.Value(comptb) <= trans_obj.Params.FrequencyEnd)
                        trans_obj.Params.FrequencyStart = surv_options_obj.FrequenciesMinToEI_FMmode.Value(comptb);
                    end
                    comptb = comptb+1;
            end
        end
        if size(surv_options_obj.FrequenciesMaxToEI_FMmode.Value,2) == num_f_fm
            switch trans_obj.Mode
                case 'FM'
                    if (surv_options_obj.FrequenciesMaxToEI_FMmode.Value(compte) <= trans_obj.Params.FrequencyEnd)||(surv_options_obj.FrequenciesMaxToEI_FMmode.Value(compte) >= trans_obj.Params.FrequencyStart)
                        trans_obj.Params.FrequencyEnd = surv_options_obj.FrequenciesMaxToEI_FMmode.Value(compte);
                    end
                    compte = compte+1;
            end
        end
    catch
        dlg_perso([],'Error','Wrong values selected for the frequency bands, check the script');
    end
    ...
    [output_2D{i_freq},out_type{i_freq},regs{i_freq},regCellInt{i_freq},shadow_height_est{i_freq}]=trans_obj.slice_transect2D_new_int(...
        ...
        'regs',regs_temp,....
        'idx_regs',[],...
        'survey_options',surv_options_obj_cp,...
        'load_bar_comp',p.Results.load_bar_comp,...
        'envdata',layer_obj.EnvData,'cal',cal_fm{i_freq},...
        'block_len',block_len,...
        'tag_sliced_output',p.Results.tag_sliced_output,...
        'keep_all',p.Results.keep_all,...
        'keep_bottom',p.Results.keep_bottom);
		
    trans_obj.Params.FrequencyStart = old_fs;
    trans_obj.Params.FrequencyEnd = old_fe;
end
...
for ireg=1:length(regs{idx_main})
    ...
    for ir=1:length(idx_freq_out_tot)
        ...
        if ~isempty(output_reg)
            if istall(output_reg.sv)
                sv_mean=gather(output_reg.sv);
            else
                sv_mean=output_reg.sv;
            end
            Sv_mean=mean(sv_mean,'all','omitnan');
            sd_sv=std(pow2db_perso(sv_mean),0,'all','omitnan');
            reg_descriptors.(sprintf('sv_%.0fkHz',layer_obj.Frequencies(idx_freq_out_tot(ir))/1e3))=Sv_mean;
            reg_descriptors.(sprintf('sd_Sv_%.0fkHz',layer_obj.Frequencies(idx_freq_out_tot(ir))/1e3))=sd_sv;
        else
        ...
        if ~isempty(output_2D{idx_main}{ity})
            ...
            if any(ity_sec)
                if ~istall(output_2D{idx_main}{ity}.eint)
                    ...
                    sv_mean=output_2D{idx_sec}{ity_sec}.sv;
                    ...
                    sv_mean=gather(output_2D{idx_sec}{ity_sec}.sv);
                ...
                output_2D{idx_main}{ity}.(sprintf('sv_%.0fkHz',layer_obj.Frequencies(idx_freq_other(ir))/1e3))=zeros(data_size);
                ...
                [mask_in,mask_out] = match_data(gather(output_2D{idx_sec}{ity_sec}.Time_S),output_2D{idx_sec}{ity_sec}.Range_ref_min,output_2D{idx_main}{ity}.Time_S,output_2D{idx_main}{ity}.Range_ref_min);
                output_2D{idx_main}{ity}.(sprintf('sv_%.0fkHz',layer_obj.Frequencies(idx_freq_other(ir))/1e3))(mask_out)=sv_mean(mask_in);
                ...
                if istall(output_2D{idx_main}{ity}.eint)
                    output_2D{idx_main}{ity}.(sprintf('sv_%.0fkHz',layer_obj.Frequencies(idx_freq_other(ir))/1e3))=tall(output_2D{idx_main}{ity}.(sprintf('sv_%.0fkHz',layer_obj.Frequencies(idx_freq_other(ir))/1e3)));
