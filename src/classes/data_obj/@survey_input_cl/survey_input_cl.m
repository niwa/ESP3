    ...
    methods
        function surv_input_obj=survey_input_cl(varargin)
            ...
            default_info=struct('Script','','XmlId','','Title','','Main_species','','Areas','','Voyage','','SurveyName','',...
                'Author',def_user,'Created',datestr(now),'Comments','');
            ...
            addParameter(p,'Options',[],@(x) isempty(x)||isa(x,'survey_options_cl'));
            ...
            if isempty(results.Options)
                surv_input_obj.Options  = survey_options_cl();
            else
                surv_input_obj.Options  = results.Options;
            end

            surv_input_obj.Infos=default_info;          
            surv_input_obj.Infos=results.Infos;
        ...
        function set.Infos(obj,infos)
            props_infos=fieldnames(infos);
            for ip=1:length(props_infos)
                switch props_infos{ip}
                    case {'Title' 'Voyage' 'SurveyName'}
                        obj.Infos.(props_infos{ip})=clean_str(infos.(props_infos{ip}));
                    otherwise
                        obj.Infos.(props_infos{ip})=infos.(props_infos{ip});
                end
            end
        end
        
        function [snapshot_vec,type_vec,stratum_vec,transect_vec,reg_num_vec,files,regs,cells,opts_cell]=list_transects(surv_in_obj)
                    ...
                    for itr=1:length(transects)
                        for itr2=1:numel(transects{itr}.number)
                            ...
                            if isfield(stratum{ist},'Options')
                                opts_cell{nb_trans} = stratum{ist}.Options;
                            else
                                opts_cell{nb_trans} = surv_in_obj.Options;
                            end
                        end
                    end
                end
