...
classdef gps_data_cl
    ...
    methods
        function obj = gps_data_cl(varargin)
            ...
            addParameter(p,'Speed',[],@isnumeric);
            ...
            if ~isempty(p.Results.Lat)
                ...
                for iprop=1:length(props)
                    if isprop(obj,props{iprop})
                        if isrow(results.(props{iprop}))
                ...
                [~,idx_sort]=unique(obj.Time);

                if numel(idx_sort)<numel(obj.Time)
                    tt = interp1(idx_sort,obj.Time(idx_sort),1:numel(obj.Time),'pchip');
                    [~,idx_sort]=unique(tt);
                    obj.Time = tt;
                end
        ...
        function obj = compute_dist_and_speed(obj)
            if numel(obj.Long)>=2
            ...
            if (isempty(obj.Speed) || all(obj.Speed == 0)) && numel(obj.Long)>=2
                [speed,time_s]=obj.speed_from_gps(min(10,numel(obj.Time)-1));
                obj.Speed=resample_data_v2(speed,time_s,obj.Time);
            elseif isempty(obj.Speed)
                obj.Speed=zeros(size(obj.Lat));
            end
        end
        ...
        function [obj_out,id_keep]=clean_gps_track(obj,varargin)
            ...
            [~,~,id_keep]=DouglasPeucker(obj.Long(id_nn),obj.Lat(id_nn),prec,0,1e2,0);
...
%             geoplot(obj.Lat,obj.Long,'k');
        ...
        function geostruct = gps_to_geostruct(obj,idx_ping)
            ...
            geostruct.Lon(geostruct.Lon >180) = geostruct.Lon(geostruct.Lon >180)-360;
            geostruct.Date        = datestr(mean(obj.Time(idx_ping))); 
        end
        ...
        function save_gps_to_file(obj,output_file,idx_ping)
            ...
            struct_obj.Speed = obj.Speed(idx_ping)';
            struct_obj.Long(struct_obj.Long >180) = struct_obj.Long(struct_obj.Long >180)-360;    
        ...
        function gps_data_section=get_GPSDData_time_section(gps_data_obj,ts,te)
            ...
            gps_data_section.Speed(idx_rem)=[];
        end
        ...
        function gps_data_section=get_GPSDData_idx_section(gps_data_obj,idx)
            ...
            gps_data_section.Speed=gps_data_obj.Speed(idx);
        end
                ...
                try
                    switch ext
                        case {'.csv','.txt'}
                            ...
                            fmt = {'dd/MM/uuuu HH:mm:ss' 'dd/MM/uuuu HH:mm:ss.SSS'};

                            for uifmt = 1:numel(fmt)
                                opts = setvaropts(opts,'Time','InputFormat',fmt{uifmt});
                                ptemp = preview(fileN{ifi},opts);
                                if ~all(isnat(ptemp.Time))
                                    break;
                                end
                            end
