classdef ac_data_cl < handle
    ...
    methods
        ...
        function obj = ac_data_cl(varargin)
            ...
            for isub = 1:length(props)
                obj.(props{isub}) = results.(props{isub});
            end
            ...
            if ~isempty(p.Results.SubData)
                ...
                for isub = 1:length(obj.SubData)
                    fieldname{isub} = obj.SubData(isub).Fieldname;
                    type{isub} = obj.SubData(isub).Type;
                end
        ...
        function ac_data_file = get_data_idx_file(ac_data_obj,file_id)
            ...
            ac_data_file.BlockId    = idx_block-min(idx_block)+1;
