...
if exist(file,'file')>0
    try
        ...
        if strcmpi(err.identifier,'MATLAB:DELETE:Permission')
            war_fig = dlg_perso([],'Could not overwrite file',sprintf('File %s is open in another process. Please close it and then close this box to continue...',file),'Timeout',60);
            waitfor(war_fig);
            delete(file);
        else
            rethrow(err);
...
infos.ESP3_version =  get_ver;
infos.date_processed = string (datetime);
info_sheet=[fieldnames(infos) struct2cell(infos)];
options=surv_obj.SurvInput.Options.surv_options_to_struct();
...
try
    ...
    if strcmpi(err.identifier,'MATLAB:table:write:FileOpenInAnotherProcess')
        war_fig = dlg_perso([],'Could not write file',sprintf('File %s is open in another process. Please close it and then close this box to continue...',file),'Timeout',60);
        waitfor(war_fig);
        writetable(cell2table(info_sheet),file,'WriteVariableNames',0,'Sheet','Options');
    else
        rethrow(err);
...
for i=1:numel(slice_trans_sum.snapshot)
    ...
    if~isempty(sheet_tot)
        ...
        try
            ...
            ind_xlsx = findstr(file,'.xlsx');
            file_bis = append(file(1:ind_xlsx),'csv');
            writetable(cell2table(sheet_tot),file_bis);
        end
...
try
    idnan = [idnan find(isnan(str_obj.time_start{idx}))];
    idnan = [idnan find(isnan(str_obj.time_end{idx}))];
catch
    idnan = [];
end
idnan = unique(idnan);
num_slices_old =  str_obj.num_slices(idx);
str_obj.num_slices(idx) = num_slices_old-size(idnan,2);
...
for i=1:numel(fields)
    ...
    if size(curr_f,2)==num_slices_old
        if idnan
            curr_f(:,idnan) = [];
        end
    end

    if isscalar(curr_f)||ischar(curr_f)
