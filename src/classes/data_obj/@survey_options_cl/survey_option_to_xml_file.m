function survey_option_to_xml_file(surv_options_obj,varargin)
p = inputParser;

addRequired(p,'surv_options_obj',@(x) isa(x,'survey_options_cl'));
...
parse(p,surv_options_obj,varargin{:});

docNode = com.mathworks.xml.XMLUtils.createDocument('survey_options');
main_node=docNode.getDocumentElement;

options_node = surv_options_obj.survey_options_to_xml_node(docNode,p.Results.subset);
