function [cal_struct_tot,ori_used]=get_transceiver_fm_cal(trans_obj,varargin)
...
eq_beam_angle_tot=trans_obj.Config.EquivalentBeamAngle';
f_nom_tot=trans_obj.Config.Frequency';
gain_tot=trans_obj.get_current_gain()';

ori = {'xml','file','th'};
ori_bool = false(size(ori));
cal_struct_tot = [];

for uif = 1:numel(eq_beam_angle_tot)
    fmin = FrequencyMinimum(uif);
    fmax = FrequencyMaximum(uif);
    gain = gain_tot(uif);
    eq_beam_angle = eq_beam_angle_tot(uif);
    f_nom = f_nom_tot(uif);


    %%%%%%%%%%%%Frequency vector (50Hz resolution)%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    cal_struct.Frequency=fmin:ceil(p.Results.f_res):fmax;
    ...
    %cal_struct.BeamWidthAlongship_th=trans_obj.Config.BeamWidthAlongship*f_nom./cal_struct.Frequency;
    %cal_struct.BeamWidthAthwartship_th=trans_obj.Config.BeamWidthAthwartship*f_nom./cal_struct.Frequency;
    ...
    if ~isfile(file_cal)
        file_cal=fullfile(p.Results.cal_path,generate_valid_filename(['Calibration_FM_' num2str(mean(f_nom),'%.0f') '.xml']));
    end
    ...
    idx_to_use = find(strcmpi(p.Results.origin ,ori));
    ...
    cal_struct_tot = [cal_struct_tot cal_struct];
end
