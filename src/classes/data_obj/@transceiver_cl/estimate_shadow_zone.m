...
%% Function
function [output_reg,slope_est,shadow_height_est,range_bathy] = estimate_shadow_zone(trans_obj,varargin)
...
addParameter(p,'Motion_correction',0,@(x) islogical(x)||isnumeric(x));
addParameter(p,'Denoised',0,@(x) islogical(x)||isnumeric(x));
...
reg_sh = trans_obj.create_WC_region(...
    ...
    'Cell_h',p.Results.Shadow_zone_height/4,...
...
output_reg = trans_obj.integrate_region(reg_sh,...
        'timeBounds',[p.Results.StartTime p.Results.EndTime],...
...
[shadow_height_est,slope_est,range_bathy] = trans_obj.get_shadow_zone_height_est();
