...
if ~isempty(erased)
    ...
    for i=1:length(erased.channel(idx_chan))
        reg=create_reg_lsss_erased(erased.channel(idx_chan),'Bad Data','LSSS Erased',range,trans_obj.TransceiverDepth);
...
for i=1:length(school)
    ...
    reg=create_reg_lsss(school(i),i,'Data','LSSS School',range,trans_obj.TransceiverDepth);
    trans_obj.add_region(reg);
end
...
for i=1:length(layer)
    ...
    reg=create_reg_lsss(layer(i),i,'Data','LSSS Layer',range,trans_obj.TransceiverDepth);
    trans_obj.add_region(reg);
end
...
Idx_ping=min(X_cont):max(X_cont);
Idx_r=min(Y_cont):max(Y_cont);
