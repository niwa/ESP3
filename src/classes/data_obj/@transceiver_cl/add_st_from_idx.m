...
idx_r=idx_r(:)';
idx_p=idx_p(:)';
single_targets=init_st_struct(numel(idx_r));
...
dt=trans_obj.get_params_value('SampleInterval',idx_p(1));
...
for ip=1:numel(idx_p)
    if any(strcmpi('AlongAngle',trans_obj.Data.Fieldname))
...
single_targets.TargetLength = Np_t;
single_targets.Track_ID = nan(size(single_targets.TargetLength));
