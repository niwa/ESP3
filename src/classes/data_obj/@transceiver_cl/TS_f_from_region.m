    ...
    case 'region_cl'
        ...
        switch p.Results.mode
            case 'max_reg'
                ...
                [Sp_max,idx_peak_red]=max(Sp_red,[],1,'omitnan');
                idx_peak=idx_r(idx_peak_red);
                pings(isnan(Sp_max))=[];
                idx_peak(isnan(Sp_max))=[];
                %       idx_peak_red(isnan(Sp_max))=[];
                range_tr=trans_obj.get_samples_range(idx_peak);   
                range_tr=range_tr';
            otherwise
                range_tr=trans_obj.get_samples_range(reg_obj.Idx_r);
...
[tmp,~,f_vec,r_tot,~]=trans_obj.processTS_f_v2(p.Results.envdata,pings(1),range_tr(:,1),p.Results.win_fact,p.Results.cal,[true true]);
...
for ip=1:length(pings)
    ...
    [Sp_f,compensation_f,f_vec,r_tot,~]=trans_obj.processTS_f_v2(p.Results.envdata,pings(ip),range_tr(:,ip),p.Results.win_fact,p.Results.cal,[true true]);
