...
trans_file_ID.Range=trans_obj.Range(1:max(trans_file_ID.Data.Nb_samples));
trans_file_ID.Spikes=trans_obj.Spikes(1:max(trans_file_ID.Data.Nb_samples),idx_file);
...
for ireg=1:numel(trans_obj.Regions)
    [reg_temp,fids]=trans_obj.Regions(ireg).split_region_per_fileID(fileID_vec,1);
    if any(fids==fileID)
...
trans_file_ID.TransceiverDepth=trans_obj.TransceiverDepth(idx_file);
