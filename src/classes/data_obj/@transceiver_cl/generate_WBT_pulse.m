...
D_1=double(max(F1.DecimationFactor,1));
D_2=double(max(F2.DecimationFactor,1));

filt_1=(F1.Coefficients(1:2:end)+1j*F1.Coefficients(2:2:end));
filt_2=(F2.Coefficients(1:2:end)+1j*F2.Coefficients(2:2:end));

t_sim_pulse=(1/f_s_ori:1/f_s_ori:pulse_length)';

np=length(t_sim_pulse);

nwtx=min(2*floor(pulse_slope*np),np);
...
sim_pulse_1=conv(sim_pulse/max(abs(sim_pulse)),filt_1,'same');
sim_pulse_1=downsample(sim_pulse_1,D_1);


sim_pulse_2=conv(sim_pulse_1/max(abs(sim_pulse_1)),filt_2,'same');
...
% figure();
% subplot(3,1,1)
% plot(t_sim_pulse*1e3,real(sim_pulse)/max(real(sim_pulse)),'b');
% grid on;
% subplot(3,1,2)
% plot(t_sim_pulse_1*1e3,real(sim_pulse_1)/max(real(sim_pulse_1)),'k');
% grid on;
% subplot(3,1,3)
% plot(t_sim_pulse_2*1e3,real(sim_pulse_2)/max(real(sim_pulse_2)),'r');
% grid on;
% xlabel('Time(ms)');
