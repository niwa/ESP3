...
classdef algo_cl < matlab.mixin.Copyable
    ...
    methods
        function obj = algo_cl(varargin)
            ...
            addParameter(p,'Input_params',[],@(x) isa(x,'input_param_cl')||isempty(x));
            ...
            obj.init_algo_input_params();

            if ~isempty(obj.Input_params)
                fields_in=obj.Input_params.get_name();
                for ifi=1:length(fields_in)
                    if isfield(results.Varargin,fields_in{ifi})
                        try
                            obj.set_input_param_value(fields_in{ifi},results.Varargin.(fields_in{ifi}));
                        catch err
        ...
        function str = get_algo_descr_and_params(obj)
            ...
            %        Precision = '%.2f'
            %        Units = ''
            str = sprintf('%s\nDescription: %s\nFunction: @%s\nInput parameters:\n',obj.Display_name, obj.Description, char(obj.Function));
        ...
        function tt = algo_params_to_table(obj)
            %        Name = ''
            %        Value= 0
            %        Validation_fcn = @isnumeric
            %        Default_value= 0
        ...
        function set_input_param_validation_fcn(obj,input_param_name,validation_fcn)
            if ~isempty(obj.Input_params)
                names=obj.Input_params.get_name();
                idx_name=find(strcmpi(input_param_name,names));
                if ~isempty(idx_name)
                    obj.Input_params(idx_name).set_validation_fcn(validation_fcn);
                end
            end
        end

        function set_input_param_value_range(obj,input_param_name,value_range)
            if ~isempty(obj.Input_params)
                names=obj.Input_params.get_name();
                idx_name=find(strcmpi(input_param_name,names));
                if ~isempty(idx_name)
                    obj.Input_params(idx_name).set_value_range(value_range);
                end
            end
        end
