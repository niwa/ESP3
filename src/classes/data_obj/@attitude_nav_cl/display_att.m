...
if ~isempty(roll)
    ...
    yyaxis(ax,'left');
    ax.YAxis(1).Color = 'r';
    a_h = plot(ax,x,heave,'r');
    ylabel(ax,'Heave (m)');
    
    
    yyaxis(ax,'right');
    a_p = plot(ax,x,pitch,'k');
    a_r = plot(ax,x,roll,'color',[0 0.5 0]);
    ax.YAxis(2).Color = 'k';
    ax.YAxis(2).TickLabelFormat  = '%g^\\circ';
    xline(ax,x(1),'--k','tag','xline');
