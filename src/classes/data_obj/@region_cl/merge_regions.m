...
for ireg=1:length(regions)
    if ~strcmp(regions(ireg).Name,'')
        ...
        for ir=i_merge_tot
            ...
            if ~strcmp(regions(ir).Name,'')&&ir~=ireg
                region_2=regions(ir);
                ...
                idx_r=floor(min(poly_combined.Vertices(:,1))):ceil(max(poly_combined.Vertices(:,1)));
                
                idx_ping=floor(min(poly_combined.Vertices(:,2))):ceil(max(poly_combined.Vertices(:,2)));
