...
if ~isempty(curr_disp)
    ...
    reverse_cmap_state = curr_disp.ReverseCmap;
    beam_angular_lim = curr_disp.BeamAngularLimit;
else
    reverse_cmap_state = 'off';
    beam_angular_lim = [-inf inf];
    cax=p.Results.Cax;
    cmap_name=p.Results.Cmap;
    cmap_list=[];
    cax_list=[];
...
if isa(trans_obj,'transceiver_cl')
    output_reg=trans_obj.integrate_region(reg_obj,'line_obj',p.Results.line_obj,'denoised',1,'load_bar_comp',p.Results.load_bar_comp,'BeamAngularLimit',beam_angular_lim);
    tt=sprintf('%s %s %.0f kHz ' ,field,p.Results.Name,trans_obj.get_params_value('FrequencyStart',1,1)/1e3 );
...
switch field
    case 'fishdensity'
        var_disp=db2pow_perso((pow2db_perso(output_reg.sv)-p.Results.TS));
        var_lin=(var_disp);
        var_scale='lin';
        ylab='(number/m3)';
    case {'sv' 'sp' 'spdenoised' 'svdenoised'}
        var_disp=pow2db_perso(output_reg.sv);
...
% title
title(ax_in,tt,'Interpreter','none');
...
if istall(y_disp)
 ...
 y  = mean(y_disp,2,'omitnan');
 
 var_disp = var_disp(~isnan(y),:);
 alphadata = alphadata(~isnan(y),:);
y  =y(~isnan(y));
% data

mat_size=size(var_disp);
if  ~any(mat_size==1)
    reg_plot=imagesc(ax_in,x_disp,y,var_disp);
...
cb.UIContextMenu=[];
cmap_struct = init_cmap(cmap_name,reverse_cmap_state);
colormap(ax_in,cmap_struct.cmap);
set(ax_in,'GridColor',cmap_struct.col_grid,'Color',cmap_struct.col_ax);

%% linear or dB scales for bottom and side displays

switch var_scale
    case 'lin'
        horz_plot=mean(var_lin,1);
        vert_plot=mean(var_lin,2);
    case 'db'
        horz_plot=pow2db_perso(mean(var_lin,1));
        vert_plot=pow2db_perso(mean(var_lin,2));
...
% data
plot(ax_vert,vert_plot,mean(y_disp,2),'r');
...
%% nest functions
    ...
    function hide_axes_cback(src,~,ax_str)
       switch ax_str 
           case 'horz'
               ax= ax_horz;
               ax_s=ax_vert;
...
% Listener for colourmap
    function listenCmapReg(~,evt)
        if ~isdeployed
            disp('listenCmapReg')
        end
        cmap_struct = init_cmap(evt.AffectedObject.Cmap);
        try
            if isvalid(ax_in)
                colormap(ax_in,cmap_struct.cmap);
                set(ax_in,'GridColor',cmap_struct.col_grid,'Color',cmap_struct.col_ax);
